...
class Disciple_Tools_AI_API {
    ...
    public static function get_ai_connection_settings(){
        ...
        return DT_AI_Network_API::get_ai_connection_settings();
    }


    public static function translate_summary( $summary ) {
        ...
        $google_api_key = $connection_settings['google_translate_api_key'] ?? '';

        if ( !empty( $google_api_key ) ) {
            $google_translation = self::translate_summary_with_google( $summary, $languages, $google_api_key );
            if ( !is_wp_error( $google_translation ) ) {
                return $google_translation;
            }
        }

        $system_prompt = 'You are an assistant helping translate a summary into multiple languages.
        Given a summary, translate it into the following languages: ' . implode( ', ', $languages ) . '.
        Return the translated summary in the following JSON format:
        {
        ';
        foreach ( $languages as $language ) {
            $system_prompt .= '"' . $language . '": "summary",';
        }
        $system_prompt .= '}';
        ...
        error_log( 'Translate Summary Execution Time: ' . $execution_time );
    ...
    private static function translate_summary_with_google( string $summary, array $languages, string $api_key ) {
        $translations = [];
        ...
        foreach ( $languages as $language ) {
            ...
            if ( empty( $translated_text ) ) {
                return new WP_Error( 'google_translation_failed', 'Google Translate did not return a translation.', [ 'status' => 500 ] );
            }
    ...
    private static function normalize_language_code( $language ) {
        ...
        [ $lang ] = array_pad( explode( '_', $language, 2 ), 1, '' );

        return $lang;
    }

    public static function summarize( string $post_type, $post_id ) {
        $post_type = sanitize_key( $post_type );
        $post_id = absint( $post_id );

        if ( empty( $post_type ) || $post_id <= 0 ) {
            return new WP_Error( 'invalid_parameters', 'A valid post_type and post_id are required.', [ 'status' => 400 ] );
        }
...
';
        ...
        $allowed_field_types = [ 'key_select', 'multi_select', 'number', 'text', 'location', 'location_meta', 'date' ];

        $prompt = "Contact Fields (Current Values):\n";
        foreach ( $contact_fields as $field => $value ) {
            if ( !empty( $value ) && isset( $field_settings[$field]['type'] ) && in_array( $field_settings[$field]['type'], $allowed_field_types, true ) ) {
                $formatted_value = self::format_field_value_to_text( $field_settings[$field]['type'], $value, $field_settings[$field] );
                $prompt .= "- {$field}: {$formatted_value}\n";
            }
        }

        $prompt .= "\nField Update History:\n";
        if ( isset( $field_updates['activity'] ) && is_array( $field_updates['activity'] ) ) {
            foreach ( $field_updates['activity'] as $update ) {
                $meta_key = $update['meta_key'] ?? '';
                if ( !empty( $meta_key ) && isset( $field_settings[$meta_key]['type'] ) && in_array( $field_settings[$meta_key]['type'], $allowed_field_types, true ) ) {
        ...
        if ( isset( $comments['comments'] ) && is_array( $comments['comments'] ) ) {
            foreach ( $comments['comments'] as $comment ) {
                $author = $comment['comment_author'] ?? 'Unknown author';
                $content = $comment['comment_content'] ?? '';
                $date = $comment['comment_date'] ?? '';
                $prompt .= "- {$author}: {$content} ({$date})\n";
            }
        }
...
//        }
        ...
        if ( isset( $field_settings['ai_summary_array'] ) ) {
            $updated = DT_Posts::update_post( $post_type, $post_id, [ 'ai_summary_array' => $summary_array ] );
    ...
    private static function format_field_value_to_text( $field_type, $value, $field_settings ) {
        switch ( $field_type ) {
            case 'key_select':
    ...
    public static function list_modules( $defaults = [
        ...
        // Delegate to network API for consistency
        return DT_AI_Network_API::list_modules( $defaults );
    }
